<template>
	<view class="content">
		...
		<view class="uni-form-item uni-column verCode">
			<input class="uni-input" placeholder-style="color:#cacaca" :value="verCodeVal" @input="chageCodeVal" maxlength="6" type="number" placeholder="请输入验证码" />
			<button class="verCodeReight" @tap="verCode" :disabled="disabled">
	...
	export default Vue.extend({
		...
		methods: {
			...
			verCode(){
				var that = this;
				this.httpRequest({
				        url : "/doctor/login/get_sms_captcha",
				        method : "POST",
						dataType:'jsonp',
				        data : {
							'mobile' : that.phoneVal
						...
						(res) => {
							if(res.data.code == 1){
								this.disabled = true;
								this.codeVel = res.data.data.sms_time;
