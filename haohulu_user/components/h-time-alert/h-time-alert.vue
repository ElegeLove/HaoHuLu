		...
		<view :class="{ appintAlertContent: true, aCShow: isShow }" @click.stop="_stopFunc">
			...
			<view class="alertTimebox">
				...
				<view class="right_box">
					<view @click="_changeTime(index)" :class="{ active: item.checked }" v-for="(item, index) in activeTimeArr" :key="item.dateStr">
	...
	methods: {
		...
		_initTime(num) {
			...
			if (num === 0 && !disabled.includes('0')) {
				//今天
				let _nowTime = new Date().getHours() + ':' + new Date().getMinutes() + ':00';
		...
		_forTime(st, et, it, dt = 0) {
			...
			let _list = [];
			if (_st < _et) {
				for (let i = _st + _it + _dayStartIntTime; i < _et; i += _it) {
					_list.push({
				...
				for (let i = _st + _it + _dayStartIntTime; i < this._toTimeStr('23:59:59'); i += _it) {
					_list.push({
						time: this._toLocalTime(i),
